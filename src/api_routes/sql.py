...
def generate_product_sql():
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).error("生成 SQL 失败: %s", e, exc_info=True)
        return jsonify({"success": False, "message": "生成 SQL 失败"}), 400
...
def generate_package_sql():
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).error("生成套餐 SQL 失败: %s", e, exc_info=True)
        return jsonify({"success": False, "message": "生成 SQL 失败"}), 400
...
def generate_offer_sql():
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).error("生成销售品 SQL 失败: %s", e, exc_info=True)
        return jsonify({"success": False, "message": "生成 SQL 失败"}), 400
