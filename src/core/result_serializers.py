...
def _fmt(v: object) -> object:
    """统一格式化输出值：Decimal → str, None → None, 其他原样返回。"""
...
def serialize_recharge_result(r) -> dict:
    return {
        ...
        "total_deduct": _fmt(r.total_deduct),
        "gifted_amount": _fmt(r.gifted_amount),
        "final_gift_amount": _fmt(r.final_gift_amount),
        "balance_before": _fmt(r.balance_before),
        "balance_after": _fmt(r.balance_after),
        "balance_diff": _fmt(r.balance_diff),
...
def serialize_suspend_result(r) -> dict:
    ...
    return {
        ...
        "balance": _fmt(r.balance),
        "plan_name": r.plan_name,
        "plan_amount": _fmt(r.plan_amount),
        "user_status": r.user_status,
        "message": r.message,
