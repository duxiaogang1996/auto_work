...
class RechargeOptions:
    ...
    bill_sleep_s: float = 0.2
    after_gift_sleep_s: float = 3.0
...
class RechargeService:
    ...
    def process_single_phone(self, *, phone: str, cookie: str, options: RechargeOptions | None = None) -> PhoneProcessResult:
    ...
    def process_phones(self, *, phones: list[str], cookie: str, options: RechargeOptions | None = None) -> list[PhoneProcessResult]:
        results: list[PhoneProcessResult] = []
        for p in phones:
            p2 = (p or "").strip()
            if not p2:
                continue
            try:
                results.append(self.process_single_phone(phone=p2, cookie=cookie, options=options))
