...
def _build_options(payload: dict) -> RechargeOptions | str:
    ...
    start_date_override = payload.get("gift_query_start_date")
    max_gift_amount_raw = payload.get("max_gift_amount")
    bill_sleep_s = float(payload.get("bill_sleep_s", 0.2) or 0)
    after_gift_sleep_s = float(payload.get("after_gift_sleep_s", 3.0) or 0)
    ...
    gift_query_start_date = str(start_date_override).strip() if start_date_override else None
    return RechargeOptions(
        dry_run=dry_run,
        max_gift_amount=max_gift_amount,
        gift_query_start_date=gift_query_start_date,
