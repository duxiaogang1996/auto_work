...
def export_cancel_account_xls():
    ...
    can_cancel_phones = [item.get("phone") for item in data if item.get("can_cancel")]

    wb = xlwt.Workbook(encoding="utf-8")
    ws = wb.add_sheet("Sheet1")

    for row_idx, phone in enumerate(can_cancel_phones):
        if phone:
            ws.write(row_idx, 0, phone)
